...
var eulerAngles = new GLMath.Euler( 0.1, -0.3, 0.25 );



var qSub = function ( a, b ) {
	var result = new GLMath.Quaternion();
	result.copy( a );
...
QUnit.test( "constructor" , function( assert ) {
	var a = new GLMath.Quaternion();
	assert.ok( a.x == 0, "Passed!" );
	assert.ok( a.y == 0, "Passed!" );
	assert.ok( a.z == 0, "Passed!" );
	assert.ok( a.w == 1, "Passed!" );

	a = new GLMath.Quaternion( x, y, z, w );
...
QUnit.test( "copy" , function( assert ) {
	var a = new GLMath.Quaternion( x, y, z, w );
	var b = new GLMath.Quaternion().copy( a );
...
QUnit.test( "setFromAxisAngle" , function( assert ) {
	...
	var zero = new GLMath.Quaternion();

	var a = new GLMath.Quaternion().setFromAxisAngle( new GLMath.Vector3( 1, 0, 0 ), 0 );
	assert.ok( a.equals( zero ), "Passed!" );
	a = new GLMath.Quaternion().setFromAxisAngle( new GLMath.Vector3( 0, 1, 0 ), 0 );
	assert.ok( a.equals( zero ), "Passed!" );
	a = new GLMath.Quaternion().setFromAxisAngle( new GLMath.Vector3( 0, 0, 1 ), 0 );
	assert.ok( a.equals( zero ), "Passed!" );

	var b1 = new GLMath.Quaternion().setFromAxisAngle( new GLMath.Vector3( 1, 0, 0 ), Math.PI );
	assert.ok( ! a.equals( b1 ), "Passed!" );
	var b2 = new GLMath.Quaternion().setFromAxisAngle( new GLMath.Vector3( 1, 0, 0 ), -Math.PI );
...
QUnit.test( "setFromEuler/setFromQuaternion", function( assert ) {
	...
	var angles = [ new GLMath.Vector3( 1, 0, 0 ), new GLMath.Vector3( 0, 1, 0 ), new GLMath.Vector3( 0, 0, 1 ) ];

	// ensure euler conversion to/from Quaternion matches.
	for( var i = 0; i < orders.length; i ++ ) {
		for( var j = 0; j < angles.length; j ++ ) {
			var eulers2 = new GLMath.Euler().setFromQuaternion( new GLMath.Quaternion().setFromEuler( new GLMath.Euler( angles[j].x, angles[j].y, angles[j].z, orders[i] ) ), orders[i] );
			var newAngle = new GLMath.Vector3( eulers2.x, eulers2.y, eulers2.z );
...
QUnit.test( "setFromEuler/setFromRotationMatrix", function( assert ) {
	...
	for( var i = 0; i < orders.length; i ++ ) {
		var q = new GLMath.Quaternion().setFromEuler( eulerAngles, orders[i] );
		var m = new GLMath.Matrix4().makeRotationFromEuler( eulerAngles, orders[i] );
		var q2 = new GLMath.Quaternion().setFromRotationMatrix( m );
...
QUnit.test( "normalize/length/lengthSq", function( assert ) {
	var a = new GLMath.Quaternion( x, y, z, w );
	var b = new GLMath.Quaternion( -x, -y, -z, -w );
...
QUnit.test( "multiplyQuaternions/multiply", function( assert ) {
	...
	var angles = [ new GLMath.Euler( 1, 0, 0 ), new GLMath.Euler( 0, 1, 0 ), new GLMath.Euler( 0, 0, 1 ) ];

	var q1 = new GLMath.Quaternion().setFromEuler( angles[0], "XYZ" );
	var q2 = new GLMath.Quaternion().setFromEuler( angles[1], "XYZ" );
	var q3 = new GLMath.Quaternion().setFromEuler( angles[2], "XYZ" );

	var q = new GLMath.Quaternion().multiplyQuaternions( q1, q2 ).multiply( q3 );

	var m1 = new GLMath.Matrix4().makeRotationFromEuler( angles[0], "XYZ" );
	var m2 = new GLMath.Matrix4().makeRotationFromEuler( angles[1], "XYZ" );
	var m3 = new GLMath.Matrix4().makeRotationFromEuler( angles[2], "XYZ" );

	var m = new GLMath.Matrix4().multiplyMatrices( m1, m2 ).multiply( m3 );

	var qFromM = new GLMath.Quaternion().setFromRotationMatrix( m );
...
QUnit.test( "multiplyVector3" , function( assert ) {
	...
	for( var i = 0; i < orders.length; i ++ ) {
		for( var j = 0; j < angles.length; j ++ ) {
			var q = new GLMath.Quaternion().setFromEuler( angles[j], orders[i] );
			var m = new GLMath.Matrix4().makeRotationFromEuler( angles[j], orders[i] );

			var v0 = new GLMath.Vector3(1, 0, 0);
...
function doSlerpObject( aArr, bArr, t ) {
	...
	var a = new GLMath.Quaternion().fromArray( aArr ),
		b = new GLMath.Quaternion().fromArray( bArr ),
		c = new GLMath.Quaternion().fromArray( aArr );
...
function doSlerpArray( a, b, t ) {
	...
	GLMath.Quaternion.slerpFlat( result, 0, a, 0, b, 0, t );
...
QUnit.test( "inverse", function ( assert ) {
	...
	var a = new GLMath.Quaternion( x, y, z, w );
	var inverted = new GLMath.Quaternion( - 0.2721655269759087, - 0.408248290463863, - 0.5443310539518174, 0.6804138174397717 );
	a.onChange( function () {
...
QUnit.test( "premultiply", function ( assert ) {
	...
	var a = new GLMath.Quaternion( x, y, z, w );
	var b = new GLMath.Quaternion( 2 * x, - y, - 2 * z, w );
	var expected = new GLMath.Quaternion( 42, - 32, - 2, 58 );
...
QUnit.test( "setFromUnitVectors", function ( assert ) {
	...
	var a = new GLMath.Quaternion();
	var b = new GLMath.Vector3( 1, 0, 0 );
	var c = new GLMath.Vector3( 0, 1, 0 );
	var expected = new GLMath.Quaternion( 0, 0, Math.sqrt( 2 ) / 2, Math.sqrt( 2 ) / 2 );
...
QUnit.test( "setFromRotationMatrix", function ( assert ) {
	...
	var a = new GLMath.Quaternion();
	var q = new GLMath.Quaternion( - 9, - 2, 3, - 4 ).normalize();
	var m = new GLMath.Matrix4().makeRotationFromQuaternion( q );
	var expected = new GLMath.Vector4( 0.8581163303210332, 0.19069251784911848, - 0.2860387767736777, 0.38138503569823695 );
	...
	q = new GLMath.Quaternion( - 1, - 2, 1, - 1 ).normalize();
	m.makeRotationFromQuaternion( q );
	expected = new GLMath.Vector4( 0.37796447300922714, 0.7559289460184544, - 0.37796447300922714, 0.37796447300922714 );
