...
QUnit.test( "at" , function( assert ) {
	var a = new GLMath.Line3( one3.clone(), new GLMath.Vector3( 1, 1, 2 ) );

	assert.ok( a.at( -1 ).distanceTo( new GLMath.Vector3( 1, 1, 0 ) ) < 0.0001, "Passed!" );
	assert.ok( a.at( 0 ).distanceTo( one3.clone() ) < 0.0001, "Passed!" );
	assert.ok( a.at( 1 ).distanceTo( new GLMath.Vector3( 1, 1, 2 ) ) < 0.0001, "Passed!" );
	assert.ok( a.at( 2 ).distanceTo( new GLMath.Vector3( 1, 1, 3 ) ) < 0.0001, "Passed!" );
});
...
QUnit.test( "closestPointToPoint/closestPointToPointParameter", function( assert ) {
	...
	assert.ok( b1.distanceTo( new GLMath.Vector3( 1, 1, 1 ) ) < 0.0001, "Passed!" );

	// nearby the ray
	assert.ok( a.closestPointToPointParameter( zero3.clone(), false ) == -1, "Passed!" );
	var b2 = a.closestPointToPoint( zero3.clone(), false );
	assert.ok( b2.distanceTo( new GLMath.Vector3( 1, 1, 0 ) ) < 0.0001, "Passed!" );

	// nearby the ray
	assert.ok( a.closestPointToPointParameter( new GLMath.Vector3( 1, 1, 5 ), true ) == 1, "Passed!" );
	var b = a.closestPointToPoint( new GLMath.Vector3( 1, 1, 5 ), true );
	assert.ok( b.distanceTo( new GLMath.Vector3( 1, 1, 2 ) ) < 0.0001, "Passed!" );
...
QUnit.test( "clone/equal", function ( assert ) {
	...
	var a = new GLMath.Line3();
	var b = new GLMath.Line3( zero3, new GLMath.Vector3( 1, 1, 1 ) );
	var c = new GLMath.Line3( zero3, new GLMath.Vector3( 1, 1, 0 ) );
...
QUnit.test( "distance", function ( assert ) {
	...
	var a = new GLMath.Line3( zero3, zero3 );
	var b = new GLMath.Line3( zero3, one3 );
	var c = new GLMath.Line3( one3.clone().negate(), one3 );
	var d = new GLMath.Line3( two3.clone().multiplyScalar( - 2 ), two3.clone().negate() );
...
QUnit.test( "applyMatrix4", function ( assert ) {
	...
	var a = new GLMath.Line3( zero3.clone(), two3.clone() );
	var b = new GLMath.Vector4( two3.x, two3.y, two3.z, 1 );
	var m = new GLMath.Matrix4().makeTranslation( x, y, z );
	var v = new GLMath.Vector3( x, y, z );

	a.applyMatrix4( m );
	assert.ok( a.start.equals( v ), "Translation: check start" );
	assert.ok( a.end.equals( new GLMath.Vector3( 2 + x, 2 + y, 2 + z ) ), "Translation: check start" );
