...
var eulerZero = new GLMath.Euler( 0, 0, 0, "XYZ" );
var eulerAxyz = new GLMath.Euler( 1, 0, 0, "XYZ" );
var eulerAzyx = new GLMath.Euler( 0, 1, 0, "ZYX" );
...
QUnit.test( "set/setFromVector3/toVector3", function( assert ) {
	...
	var vec = new GLMath.Vector3( 0, 1, 0 );

	var b = new GLMath.Euler().setFromVector3( vec, "ZYX" );
...
QUnit.test( "Quaternion.setFromEuler/Euler.fromQuaternion", function( assert ) {
	...
	for( var i = 0; i < testValues.length; i ++ ) {
		var v = testValues[i];
		var q = new GLMath.Quaternion().setFromEuler( v );

		var v2 = new GLMath.Euler().setFromQuaternion( q, v.order );
		var q2 = new GLMath.Quaternion().setFromEuler( v2 );
...
QUnit.test( "reorder" , function( assert ) {
	...
	for( var i = 0; i < testValues.length; i ++ ) {
		...
		v.reorder( 'YZX' );
		var q2 = new GLMath.Quaternion().setFromEuler( v );
		assert.ok( quatEquals( q, q2 ), "Passed!" );

		v.reorder( 'ZXY' );
		var q3 = new GLMath.Quaternion().setFromEuler( v );
...
QUnit.test( "gimbalLocalQuat" , function( assert ) {
	...
	var q1 = new GLMath.Quaternion( 0.5207769385244341, -0.4783214164122354, 0.520776938524434, 0.47832141641223547 );
	var q2 = new GLMath.Quaternion( 0.11284905712620674, 0.6980437630368944, -0.11284905712620674, 0.6980437630368944 );

	var eulerOrder = "ZYX";

	// create Euler directly from a Quaternion
	var eViaQ1 = new GLMath.Euler().setFromQuaternion( q1, eulerOrder ); // there is likely a bug here

	// create Euler from Quaternion via an intermediate Matrix4
	var mViaQ1 = new GLMath.Matrix4().makeRotationFromQuaternion( q1 );
	var eViaMViaQ1 = new GLMath.Euler().setFromRotationMatrix( mViaQ1, eulerOrder );
...
QUnit.test( "clone/copy, check callbacks", function ( assert ) {
	...
	var a = new GLMath.Euler( 1, 2, 3, "ZXY" );
	var b = new GLMath.Euler( 4, 5, 6, "XZY" );
	var cb = function () {
...
QUnit.test( "toArray", function ( assert ) {
	...
	var order = "YXZ";
	var a = new GLMath.Euler( x, y, z, order );
