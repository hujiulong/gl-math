...
QUnit.test( "at" , function( assert ) {
	var a = new GLMath.Ray( one3.clone(), new GLMath.Vector3( 0, 0, 1 ) );

	assert.ok( a.at( 0 ).equals( one3 ), "Passed!" );
	assert.ok( a.at( -1 ).equals( new GLMath.Vector3( 1, 1, 0 ) ), "Passed!" );
	assert.ok( a.at( 1 ).equals( new GLMath.Vector3( 1, 1, 2 ) ), "Passed!" );
});
...
QUnit.test( "recast/clone", function( assert ) {
	...
	var b = a.clone();
	assert.ok( b.recast( -1 ).equals( new GLMath.Ray( new GLMath.Vector3( 1, 1, 0 ), new GLMath.Vector3( 0, 0, 1 ) ) ), "Passed!" );

	var c = a.clone();
	assert.ok( c.recast( 1 ).equals( new GLMath.Ray( new GLMath.Vector3( 1, 1, 2 ), new GLMath.Vector3( 0, 0, 1 ) ) ), "Passed!" );
...
QUnit.test( "closestPointToPoint" , function( assert ) {
	...
	// front of the ray
	var c = a.closestPointToPoint( new GLMath.Vector3( 0, 0, 50 ) );
	assert.ok( c.equals( new GLMath.Vector3( 1, 1, 50 ) ), "Passed!" );
...
QUnit.test( "distanceToPoint" , function( assert ) {
	...
	// front of the ray
	var c = a.distanceToPoint( new GLMath.Vector3( 0, 0, 50 ) );
...
QUnit.test( "distanceSqToPoint" , function( assert ) {
	...
	// front of the ray
	var c = a.distanceSqToPoint( new GLMath.Vector3( 0, 0, 50 ) );
...
QUnit.test( "intersectsSphere" , function( assert ) {
	var a = new GLMath.Ray( one3.clone(), new GLMath.Vector3( 0, 0, 1 ) );
	var b = new GLMath.Sphere( zero3, 0.5 );
	var c = new GLMath.Sphere( zero3, 1.5 );
	var d = new GLMath.Sphere( one3, 0.1 );
	var e = new GLMath.Sphere( two3, 0.1 );
	var f = new GLMath.Sphere( two3, 1 );
...
QUnit.test( "intersectSphere" , function( assert ) {
	...
	var a0 = new GLMath.Ray( zero3.clone(), new GLMath.Vector3( 0, 0, -1 ) );
	// ray a1 origin located at ( 1, 1, 1 ) and points left in negative-x direction
	var a1 = new GLMath.Ray( one3.clone(), new GLMath.Vector3( -1, 0, 0 ) );

	// sphere (radius of 2) located behind ray a0, should result in null
	var b = new GLMath.Sphere( new GLMath.Vector3( 0, 0, 3 ), 2 );
	...
	var b = new GLMath.Sphere( new GLMath.Vector3( 3, 0, -1 ), 2 );
	assert.ok( a0.intersectSphere( b ) === null, "Passed!" );

	// sphere (radius of 2) located below ray a1, should result in null
	var b = new GLMath.Sphere( new GLMath.Vector3( 1, -2, 1 ), 2 );
	...
	var b = new GLMath.Sphere( new GLMath.Vector3( -1, 1, 1 ), 1 );
	assert.ok( a1.intersectSphere( b ).distanceTo( new GLMath.Vector3( 0, 1, 1 ) ) < TOL, "Passed!" );

	// sphere (radius of 1) located in front of ray a0, should result in intersection at 0, 0, -1
	var b = new GLMath.Sphere( new GLMath.Vector3( 0, 0, -2 ), 1 );
	assert.ok( a0.intersectSphere( b ).distanceTo( new GLMath.Vector3( 0, 0, -1 ) ) < TOL, "Passed!" );

	// sphere (radius of 2) located in front & right of ray a0, should result in intersection at 0, 0, -1, or left-most edge of sphere
	var b = new GLMath.Sphere( new GLMath.Vector3( 2, 0, -1 ), 2 );
	assert.ok( a0.intersectSphere( b ).distanceTo( new GLMath.Vector3( 0, 0, -1 ) ) < TOL, "Passed!" );

	// same situation as above, but move the sphere a fraction more to the right, and ray a0 should now just miss
	var b = new GLMath.Sphere( new GLMath.Vector3( 2.01, 0, -1 ), 2 );
	assert.ok( a0.intersectSphere( b ) === null, "Passed!" );
	...
	// thus keeping the intersection point always in front of the ray.
	var b = new GLMath.Sphere( new GLMath.Vector3( 0, 0, 1 ), 4 );
	assert.ok( a0.intersectSphere( b ).distanceTo( new GLMath.Vector3( 0, 0, -3 ) ) < TOL, "Passed!" );
	...
	// thus keeping the intersection point always in front of the ray.
	var b = new GLMath.Sphere( new GLMath.Vector3( 0, 0, -1 ), 4 );
	assert.ok( a0.intersectSphere( b ).distanceTo( new GLMath.Vector3( 0, 0, -5 ) ) < TOL, "Passed!" );
...
QUnit.test( "intersectsPlane" , function( assert ) {
	...
	var b = new GLMath.Plane().setFromNormalAndCoplanarPoint( new GLMath.Vector3( 0, 0, 1 ), one3.clone().sub( new GLMath.Vector3( 0, 0, -1 ) ) );
	assert.ok( a.intersectsPlane( b ), "Passed!" );

	// parallel plane coincident with origin
	var c = new GLMath.Plane().setFromNormalAndCoplanarPoint( new GLMath.Vector3( 0, 0, 1 ), one3.clone().sub( new GLMath.Vector3( 0, 0, 0 ) ) );
	assert.ok( a.intersectsPlane( c ), "Passed!" );

	// parallel plane behind the ray
	var d = new GLMath.Plane().setFromNormalAndCoplanarPoint( new GLMath.Vector3( 0, 0, 1 ), one3.clone().sub( new GLMath.Vector3( 0, 0, 1 ) ) );
	assert.ok( ! a.intersectsPlane( d ), "Passed!" );

	// perpendical ray that overlaps exactly
	var e = new GLMath.Plane().setFromNormalAndCoplanarPoint( new GLMath.Vector3( 1, 0, 0 ), one3 );
	assert.ok( a.intersectsPlane( e ), "Passed!" );

	// perpendical ray that doesn't overlap
	var f = new GLMath.Plane().setFromNormalAndCoplanarPoint( new GLMath.Vector3( 1, 0, 0 ), zero3 );
...
QUnit.test( "intersectPlane" , function( assert ) {
	...
	var b = new GLMath.Plane().setFromNormalAndCoplanarPoint( new GLMath.Vector3( 0, 0, 1 ), new GLMath.Vector3( 1, 1, -1 ) );
	assert.ok( a.intersectPlane( b ) === null, "Passed!" );

	// parallel plane coincident with origin
	var c = new GLMath.Plane().setFromNormalAndCoplanarPoint( new GLMath.Vector3( 0, 0, 1 ), new GLMath.Vector3( 1, 1, 0 ) );
	assert.ok( a.intersectPlane( c ) === null, "Passed!" );

	// parallel plane infront
	var d = new GLMath.Plane().setFromNormalAndCoplanarPoint( new GLMath.Vector3( 0, 0, 1 ), new GLMath.Vector3( 1, 1, 1 ) );
...
QUnit.test( "applyMatrix4" , function( assert ) {
	...
	a = new GLMath.Ray( zero3.clone(), new GLMath.Vector3( 0, 0, 1 ) );
	...
	a.origin = new GLMath.Vector3( 0, 0, 1 );
	b.origin = new GLMath.Vector3( 0, 0, -1 );
	var a2 = a.clone().applyMatrix4( m );
	assert.ok( a2.origin.distanceTo( b.origin ) < 0.0001, "Passed!" );
	assert.ok( a2.direction.distanceTo( b.direction ) < 0.0001, "Passed!" );
});
...
QUnit.test( "distanceSqToSegment" , function( assert ) {
	var a = new GLMath.Ray( one3.clone(), new GLMath.Vector3( 0, 0, 1 ) );
	var ptOnLine = new GLMath.Vector3();
	var ptOnSegment = new GLMath.Vector3();

	//segment in front of the ray
	var v0 = new GLMath.Vector3( 3, 5, 50 );
	var v1 = new GLMath.Vector3( 50, 50, 50 ); // just a far away point
	var distSqr = a.distanceSqToSegment( v0, v1, ptOnLine, ptOnSegment );

	assert.ok( ptOnSegment.distanceTo( v0 ) < 0.0001, "Passed!" );
	assert.ok( ptOnLine.distanceTo( new GLMath.Vector3(1, 1, 50) ) < 0.0001, "Passed!" );
	// ((3-1) * (3-1) + (5-1) * (5-1) = 4 + 16 = 20
	assert.ok( Math.abs( distSqr - 20 ) < 0.0001, "Passed!" );

	//segment behind the ray
	v0 = new GLMath.Vector3( -50, -50, -50 ); // just a far away point
	v1 = new GLMath.Vector3( -3, -5, -4 );
	...
	v0 = new GLMath.Vector3( -50, -50, -50 );
	v1 = new GLMath.Vector3( 50, 50, 50 );
	distSqr = a.distanceSqToSegment( v0, v1, ptOnLine, ptOnSegment );
...
QUnit.test( "intersectBox" , function( assert ) {
	...
	var box = new GLMath.Box3( new GLMath.Vector3(  -1, -1, -1 ), new GLMath.Vector3( 1, 1, 1 ) );

	var a = new GLMath.Ray( new GLMath.Vector3( -2, 0, 0 ), new GLMath.Vector3( 1, 0, 0) );
	//ray should intersect box at -1,0,0
	assert.ok( a.intersectsBox(box) === true, "Passed!" );
	assert.ok( a.intersectBox(box).distanceTo( new GLMath.Vector3( -1, 0, 0 ) ) < TOL, "Passed!" );

	var b = new GLMath.Ray( new GLMath.Vector3( -2, 0, 0 ), new GLMath.Vector3( -1, 0, 0) );
	...
	var c = new GLMath.Ray( new GLMath.Vector3( 0, 0, 0 ), new GLMath.Vector3( 1, 0, 0) );
	// ray is inside box, should return exit point
	assert.ok( c.intersectsBox(box) === true, "Passed!" );
	assert.ok( c.intersectBox(box).distanceTo( new GLMath.Vector3( 1, 0, 0 ) ) < TOL, "Passed!" );

	var d = new GLMath.Ray( new GLMath.Vector3( 0, 2, 1 ), new GLMath.Vector3( 0, -1, -1).normalize() );
	//tilted ray should intersect box at 0,1,0
	assert.ok( d.intersectsBox(box) === true, "Passed!" );
	assert.ok( d.intersectBox(box).distanceTo( new GLMath.Vector3( 0, 1, 0 ) ) < TOL, "Passed!" );

	var e = new GLMath.Ray( new GLMath.Vector3( 1, -2, 1 ), new GLMath.Vector3( 0, 1, 0).normalize() );
	...
	assert.ok( e.intersectBox(box).distanceTo( new GLMath.Vector3( 1, -1, 1 ) ) < TOL, "Passed!" );

	var f = new GLMath.Ray( new GLMath.Vector3( 1, -2, 0 ), new GLMath.Vector3( 0, -1, 0).normalize() );
...
QUnit.test( "lookAt", function ( assert ) {
	...
	var a = new GLMath.Ray( two3.clone(), one3.clone() );
...
QUnit.test( "intersectTriangle", function ( assert ) {
	...
	var ray = new GLMath.Ray();
	var a = new GLMath.Vector3( 1, 1, 0 );
	var b = new GLMath.Vector3( 0, 1, 1 );
	var c = new GLMath.Vector3( 1, 0, 1 );
	var intersect;
