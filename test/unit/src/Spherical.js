...
QUnit.test( "constructor", function ( assert ) {
	...
	a = new GLMath.Spherical( radius, phi, theta );
...
QUnit.test( "clone", function ( assert ) {
	...
	var phi = Math.acos( - 0.5 );
	var theta = Math.sqrt( Math.PI ) * phi;
	var a = new GLMath.Spherical( radius, phi, theta );
	var b = a.clone();
...
QUnit.test( "copy", function ( assert ) {
	...
	var theta = Math.sqrt( Math.PI ) * phi;
	var a = new GLMath.Spherical( radius, phi, theta );
	var b = new GLMath.Spherical().copy( a );
...
QUnit.test( "makeSafe", function ( assert ) {
	...
	var tooHigh = Math.PI;
	var justRight = 1.5;
	var a = new GLMath.Spherical( 1, tooLow, 0 );
...
QUnit.test( "setFromVector3", function ( assert ) {
	...
	var a = new GLMath.Spherical( 1, 1, 1 );
	var b = new GLMath.Vector3( 0, 0, 0 );
	var c = new GLMath.Vector3( Math.PI, 1, - Math.PI );
	var expected = new GLMath.Spherical( 4.554032147688322, 1.3494066171539107, 2.356194490192345 );
